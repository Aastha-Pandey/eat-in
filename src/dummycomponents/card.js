...
export function Card(props) {
  ...
  useEffect(() => {
    getOffers(props.item.id).then((data) => {
      setOffers(data.data);
    });
    getCuisines(props.item.id).then((data) => {
      setCuisines(data.data);
    });

    getAverageRatingOfRestaurants(props.item.city, props.item.id)
      .then((data) => {
        setRatings(data.data);
      })
      .catch((data) => {
        setRatings(data.errors);
      });
  }, [props.item.city, props.item.id]);
